...
def get_date_id(db: Session, date_value: datetime) -> int:
    date_record = (
        db.query(DimDate).filter(DimDate.date_value == str(date_value)).first()
    )
    if not date_record:
        raise HTTPException(
            status_code=400,
            detail=f"No matching date_id found for {date_value}. Ensure the date exists in dim_date.",
        )
    ...
    try:
        ...
        data = [
            AdMetricsResponse(
                ...
                device_type=(
                    result.device_type.device_type_name if result.device_type else None
                ),
